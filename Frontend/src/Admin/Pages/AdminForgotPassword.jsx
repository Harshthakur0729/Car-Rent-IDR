...
import { Mail, User, ArrowRight, Activity, Hexagon, ChevronLeft, CheckCircle, AlertCircle, ShieldAlert, KeyRound } from 'lucide-react';
...
const AdminForgotPassword = () => {
  ...
  const [userDetails, setUserDetails] = useState({ adminname: "", email: "" });
  ...
  const handleForm = async (e) => {
    ...
    try {
      ...
      setCustomAlert({
        ...
        message: res.data.message || "Secure OTP sent to your email frequency."
      });
  ...
  return (
    <div className="min-h-screen w-full bg-black text-white flex flex-col lg:flex-row overflow-hidden selection:bg-red-500 selection:text-black relative font-sans">
            ...
            >
              ...
              <div className="relative z-10 mb-4 flex justify-center">
                {customAlert.type === 'success' ? <CheckCircle size={64} className="text-green-500" /> :
                  customAlert.type === 'warning' ? <ShieldAlert size={64} className="text-yellow-500" /> :
      ...
      <div className="relative w-full lg:w-[60%] h-[40vh] lg:h-screen overflow-hidden group">
        {/* Background Image (Red Theme for Admin) */}
        ...
        >
          ...
          <p className="text-gray-300 mt-2 text-sm lg:text-base max-w-md hidden sm:block">
            Initiate secure protocol to recover command access. Authentication required.
          </p>
      ...
      >
        ...
        <motion.div variants={formVariants} className="w-full max-w-md relative z-20">
          ...
          {/* Back Link */}
          <Link to="/login" className="inline-flex items-center text-zinc-500 hover:text-red-500 text-xs font-mono mb-8 transition-colors">
          ...
          <div className="mb-10 text-center lg:text-left">
            ...
            <p className="text-zinc-500 text-sm mt-1">Enter your admin details to receive OTP.</p>
          </div>
          ...
          <form onSubmit={handleForm} className="space-y-6">
            ...
            <div className="relative mt-8">
              <label className={`absolute left-0 -top-3 text-[10px] font-mono tracking-widest transition-all duration-300 ${focusedField === 'email' ? 'text-red-500' : 'text-zinc-500'}`}>
                ADMIN EMAIL ADDRESS
              </label>
              <div className={`flex items-center border-b-2 py-3 transition-all duration-300 ${focusedField === 'email' ? 'border-red-500 bg-red-500/5' : 'border-zinc-800'}`}>
                <Mail size={20} className={`mr-3 transition-colors ${focusedField === 'email' ? 'text-red-500' : 'text-zinc-600'}`} />
                <input
                  type="email"
                  name="email"
