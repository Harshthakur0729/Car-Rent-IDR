...
const AvailableCar = () => {
  ...
  const Booknow = (e, car) => {
    ...
    // Check verified state directly from loaded data
    if (verifyUser && verifyUser.cardverify === true) {
      navigate('/book-car', { state: car });
    } else {
      setAlert({
        show: true,
        message: "Your identity document is not verified. Please verify your document first to proceed."
      });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-cyan-500 selection:text-black overflow-x-hidden ">
      ...
      {/*  ALERT MODAL (Document Verification)  */}
      <AnimatePresence>
        {alert.show && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
            <motion.div
              ...
              className="glass-alert w-full max-w-md p-8 rounded-2xl text-center relative"
            >
              <div className="w-16 h-16 bg-red-900/50 rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-red-500 animate-pulse">
              ...
              <button
                ...
                className="w-full py-3 bg-red-600 hover:bg-red-500 text-white font-bold text-xs tracking-widest uppercase rounded-lg shadow-lg shadow-red-900/40 transition-all"
              >
                Verify Document Now
              </button>
              <button
                onClick={() => setAlert({ show: false, message: '' })}
                className="mt-3 text-zinc-500 text-xs hover:text-white underline"
              >
                Close
              </button>
            </motion.div>
        ...
        <div className="absolute inset-0 z-0 overflow-hidden">
          <div className="absolute top-1/2 left-1/2 w-[300%] h-[300%] -translate-x-1/2 -translate-y-1/2 pointer-events-none">
            <iframe
              ...
              src={videoUrl}
              title="Garage Background" frameBorder="0"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {visibleCars.length > 0 ? (
            visibleCars.map((rawCar, index) => {
              //  DATA TRANSFORMATION (MAPPING HERE) 
              const car = {
                ...rawCar,
                // Fallback for images
              ...
              return (
                <div
                  ...
                  className={`group relative h-[450px] bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800 hover:border-cyan-500/50 transition-all duration-500 animate-fade-in hover:shadow-[0_0_40px_rgba(6,182,212,0.1)] cursor-pointer`}
                ...
                >
                  ...
                  <div className="h-3/5 w-full overflow-hidden relative bg-zinc-950">
                    ...
                    <div className="absolute top-4 right-4 bg-black/70 backdrop-blur-md px-3 py-1 rounded text-[10px] font-mono text-cyan-400 border border-cyan-500/30 uppercase tracking-wider">
                      {car.type || "LUXURY"}
                    </div>
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-6 bg-zinc-900 border-t border-zinc-800/50">
                    ...
                    <div className="flex gap-3">
                      <button className="flex-1 py-3 bg-zinc-800 border border-zinc-700 hover:bg-cyan-900/40 hover:border-cyan-500 hover:text-cyan-400 text-center text-xs font-bold tracking-widest text-white transition-all uppercase rounded-lg">
                        View Details
                      </button>
                      <button
                        onClick={(e) => {
                        ...
                        className="flex-1 py-3 bg-cyan-600 hover:bg-cyan-500 text-black text-center text-xs font-bold tracking-widest transition-all uppercase rounded-lg shadow-lg hover:shadow-cyan-500/50"
                      >
                        Book Now
                      </button>
                    </div>
        ...
        {selectedCar && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-xl p-4" onClick={closeModal}>
            <motion.div
              ...
              className="glass-modal w-full max-w-6xl rounded-3xl overflow-hidden relative flex flex-col lg:flex-row max-h-[90vh] lg:h-[80vh]"
            >
              <button onClick={closeModal} className="absolute top-4 right-4 z-50 bg-black/50 text-white p-2 rounded-full hover:bg-red-600 transition-colors">
                <X size={24} />
              </button>
              ...
              <div className="w-full lg:w-[55%] bg-black relative group flex flex-col h-[40vh] lg:h-auto">
                ...
                <div className="flex-1 relative overflow-hidden bg-zinc-950 flex items-center justify-center">
                  <img
                    ...
                    alt={`${selectedCar.name} view ${currentImageIndex}`}
                  ...
                  {selectedCar.images?.length > 1 && (
                    <>
                      <button onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/50 p-3 rounded-full text-white hover:bg-cyan-600 transition-all"><ChevronLeft size={24} /></button>
                      <button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/50 p-3 rounded-full text-white hover:bg-cyan-600 transition-all"><ChevronRight size={24} /></button>
                    </>
                  )}
                </div>

                {/* Thumbnails Strip (Auto-Scrolling) */}
                {selectedCar.images?.length > 1 && (
                  <div
                    ref={thumbnailRef}
                    className="h-24 bg-zinc-950 flex items-center gap-3 px-4 overflow-x-auto gallery-scroll border-t border-zinc-800 shrink-0"
                  >
                    {selectedCar.images.map((img, idx) => (
                      <div
                        key={idx}
                        onClick={() => selectImage(idx)}
                        className={`h-16 w-24 flex-shrink-0 rounded-lg overflow-hidden cursor-pointer border-2 transition-all duration-300 ${currentImageIndex === idx ? 'border-cyan-500 opacity-100 scale-105' : 'border-transparent opacity-40 hover:opacity-100'}`}
                      >
              ...
              <div className="w-full lg:w-[45%] p-8 bg-zinc-900/95 border-l border-zinc-800 flex flex-col overflow-y-auto custom-scrollbar">
                ...
                <div className="mb-8">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="px-3 py-1 bg-cyan-900/30 text-cyan-400 text-[10px] font-bold rounded border border-cyan-900/50 uppercase tracking-widest">{selectedCar.brand}</span>
                    <span className="text-zinc-500 text-xs font-mono">Model {selectedCar.year}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-zinc-800">
                    <div>
                      <span className="block text-3xl font-bold text-white">${Number(selectedCar.price).toLocaleString()}</span>
                      <span className="text-xs text-zinc-500 font-mono tracking-widest uppercase">PER DAY</span>
                    </div>
                    <button
                      onClick={(e) => {
                        closeModal();
                        Booknow(e, selectedCar);
                      }}
                      className="px-8 py-4 bg-white text-black font-bold text-xs rounded-lg uppercase tracking-widest hover:bg-cyan-400 transition-all shadow-[0_0_30px_rgba(255,255,255,0.2)] flex items-center gap-2 group"
                    >
                      <span>Book Now</span> <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
                    </button>
                  </div>
                </div>
                ...
                {/* Features Badges */}
                <div className="mb-8">
                  <h3 className="text-xs font-bold text-cyan-500 uppercase tracking-widest mb-3 flex items-center gap-2"><Info size={12} /> Premium Features</h3>
                  <div className="flex flex-wrap gap-2">
                    {selectedCar.AC && <span className="px-3 py-1.5 bg-zinc-800/50 border border-zinc-700 rounded-lg text-[10px] text-zinc-300 font-bold uppercase flex items-center gap-1"><Wind size={10} /> AC</span>}
                    {selectedCar.GPS && <span className="px-3 py-1.5 bg-zinc-800/50 border border-zinc-700 rounded-lg text-[10px] text-zinc-300 font-bold uppercase flex items-center gap-1"><MapPin size={10} /> GPS Navigation</span>}
                    {selectedCar.musicSystem && <span className="px-3 py-1.5 bg-zinc-800/50 border border-zinc-700 rounded-lg text-[10px] text-zinc-300 font-bold uppercase flex items-center gap-1"><Music size={10} /> Premium Audio</span>}
                    {selectedCar.airbags && <span className="px-3 py-1.5 bg-zinc-800/50 border border-zinc-700 rounded-lg text-[10px] text-zinc-300 font-bold uppercase flex items-center gap-1"><ShieldCheck size={10} /> Airbags</span>}
                    {selectedCar.sunroof && <span className="px-3 py-1.5 bg-zinc-800/50 border border-zinc-700 rounded-lg text-[10px] text-zinc-300 font-bold uppercase flex items-center gap-1"><Sun size={10} /> Sunroof</span>}
                  </div>
                  {(!selectedCar.AC && !selectedCar.GPS && !selectedCar.musicSystem && !selectedCar.airbags && !selectedCar.sunroof) && (
                    <p className="text-xs text-zinc-600 italic">Standard configuration.</p>
                  )}
                </div>

                {/* Description */}
                <div>
                  <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-3 flex items-center gap-2"><DollarSign size={12} /> Vehicle Overview</h3>
                  <p className="text-sm text-zinc-300 leading-relaxed font-light border-l-2 border-cyan-500/30 pl-4">
